<?php
	...
	if($thisMD5==$prevMD5){
		...
		// set up tables
		setupTable('customers', "create table if not exists `customers` (   `CustomerID` VARCHAR(5) not null , primary key (`CustomerID`), `CompanyName` VARCHAR(40) , `ContactName` VARCHAR(30) , `ContactTitle` VARCHAR(30) , `Address` TEXT , `City` VARCHAR(15) , `Region` VARCHAR(15) , `PostalCode` VARCHAR(10) , `Country` VARCHAR(15) , `Phone` VARCHAR(24) , `Fax` VARCHAR(24) ) CHARSET latin1", $silent);
		...
		setupTable('categories', "create table if not exists `categories` (   `CategoryID` INT not null auto_increment , primary key (`CategoryID`), `Picture` VARCHAR(40) , `CategoryName` VARCHAR(50) , unique `CategoryName_unique` (`CategoryName`), `Description` TEXT ) CHARSET latin1", $silent);
